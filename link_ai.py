...
class LinkAI:
    ...
    def content_plan(self, prompt, info):
        ...
        response = client.responses.create(
            ...
            input=[{
                "role": "system",
                "content": f"""Создай контент-план для социальных сетей НКО [Название НКО взять из информации после символа - ㅤ встреченного позже] на период [указать временной промежуток, например, месяц] с учётом следующих параметров:
