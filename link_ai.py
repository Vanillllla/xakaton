    ...
    def content_plan(self, prompt, info):
        ...
        response = client.responses.create(
            ...
            input=[{
                "role": "system",
                "content": f"""
Создай контент-план для социальных сетей Не Коммерчекой Организации [Название НКО взять из информации после символа ㅤ встреченного позже] 
...
Ты не должен давать нейросети задачу, а должен выписать роль.
Сделай роль так, чтобы нейросеть при создании контента для Не Коммерческой Организации учитывала полученные данные. В твоей работе:
Важно сохранить фактические данные предоставленные пользователем, в том числе хэштеги.
Нейросети важно использовать подходящие хештэги.
Нейросеть будет получать вместе с твоим ещё и пользовательский промт, поэтому оставь в промте фактические данные. 
Нейросеть не может задать вопрос в процессе диалога, все данные предоставлены до него в полученном тобой сообщении"""
    ...
    def prompt_from_settings(self, settings: dict) -> str:
        ...
        return f"Пиши в стиле:{style}, в тоне: {tone}, около {size} слов. Не уточняй по поводу вышеперечисленных пунктов и сконцентрируйся на вводе пользователя. Далее следует информация об организации."
